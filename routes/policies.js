...
// Precios dinámicos
async function getCoveragePricing(coverageName) {
  const coverage = await queryGet('SELECT * FROM coverages WHERE nombre = ? AND active = 1', [coverageName]);
  if (!coverage) return { prima: 0, monto: 0 };
  const prima = coverage.prima || 0;
  let monto = 0;
  for (let i = 1; i <= 10; i++) {
    monto += Number(coverage[`cob_${i}_monto`] || 0);
  }
  return { prima, monto };
}

// Generador de número de póliza
async function generatePolicyNumber() {
  const seqName = 'policy';
  let seq = await queryGet('SELECT value FROM sequences WHERE name = ?', [seqName]);
  
  if (!seq) {
    const maxRow = await queryGet(`SELECT MAX(CAST(RIGHT(policy_number, 8) AS UNSIGNED)) as maxNum FROM policies`);
    const startVal = maxRow && maxRow.maxNum ? maxRow.maxNum : 0;
    await queryRun('INSERT INTO sequences (name, value) VALUES (?, ?)', [seqName, startVal]);
    seq = { value: startVal };
  }

  const nextNum = seq.value + 1;
  await queryRun('UPDATE sequences SET value = ? WHERE name = ?', [nextNum, seqName]);
...
router.get('/', async (req, res) => {
  try {
    ...
    let query = `SELECT p.*, CONCAT(c.nombre, ' ', c.apellido) as client_name, c.cedula as client_cedula 
    ...
    const policies = await queryAll(query, params);
...
router.post('/', async (req, res) => {
  try {
    ...
    const client = await queryGet('SELECT id FROM clients WHERE id = ? AND active = 1', [client_id]);
    if (!client) return res.status(404).json({ error: 'Cliente no encontrado.' });

    const policy_number = await generatePolicyNumber();
    const { prima, monto } = await getCoveragePricing(cobertura);
    ...
    const policy = await queryGet('SELECT * FROM policies WHERE id = ?', [result.lastInsertRowid]);
...
router.put('/:id', async (req, res) => {
  try {
    const policy = await queryGet('SELECT * FROM policies WHERE id = ?', [req.params.id]);
    if (!policy) return res.status(404).json({ error: 'Póliza no encontrada.' });
    ...
    if (cobertura && cobertura !== policy.cobertura) {
      const pricing = await getCoveragePricing(cobertura);
      if (pricing.prima > 0) {
    ...
    await queryRun(
      ...
      WHERE id = ?`,
      [
        estado || policy.estado, notas !== undefined ? notas : policy.notas, cobertura || policy.cobertura, fecha_fin || policy.fecha_fin,
        newPrima, newMonto, placa ? placa.toUpperCase() : policy.placa, marca || policy.marca, modelo || policy.modelo,
        anio || policy.anio, color || policy.color, serial_carroceria !== undefined ? serial_carroceria : policy.serial_carroceria,
        serial_motor !== undefined ? serial_motor : policy.serial_motor, req.params.id
      ]
    );

    const updated = await queryGet(
      `SELECT p.*, CONCAT(c.nombre, ' ', c.apellido) as client_name FROM policies p JOIN clients c ON p.client_id = c.id WHERE p.id = ?`,
      [req.params.id]
    );
...
router.post('/:id/renew', async (req, res) => {
  try {
    const policy = await queryGet(`SELECT p.*, CONCAT(c.nombre, ' ', c.apellido) as client_name FROM policies p JOIN clients c ON p.client_id = c.id WHERE p.id = ?`, [req.params.id]);
    if (!policy) return res.status(404).json({ error: 'Póliza no encontrada.' });
    ...
    const { prima, monto } = await getCoveragePricing(policy.cobertura);

    await queryRun(
      `UPDATE policies SET fecha_inicio = ?, fecha_fin = ?, prima = ?, monto = ?, estado = 'activa' WHERE id = ?`,
      [fecha_inicio, fecha_fin, prima || policy.prima, monto || policy.monto, req.params.id]
    );

    const updated = await queryGet(`SELECT p.*, CONCAT(c.nombre, ' ', c.apellido) as client_name, c.cedula as client_cedula FROM policies p JOIN clients c ON p.client_id = c.id WHERE p.id = ?`, [req.params.id]);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await queryRun('DELETE FROM policies WHERE id = ?', [req.params.id]);
...
router.get('/config/pricing', async (req, res) => {
  try {
    const coverages = await queryAll('SELECT * FROM coverages WHERE active = 1 ORDER BY nombre ASC');
    const pricing = {};
    coverages.forEach(c => {
      let totalMonto = 0;
      const items = [];
      for (let i = 1; i <= 10; i++) {
        ...
        const monto = Number(c[`cob_${i}_monto`] || 0);
        if (name) { items.push({ nombre: name, monto }); totalMonto += monto; }
      }
      pricing[c.nombre] = { prima: c.prima, monto: totalMonto, descripcion: c.descripcion, items };
    });
    res.json({ pricing });
  } catch (err) {
